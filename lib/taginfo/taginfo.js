...
const getOtherKeys = (data) => {
    if (data.matches_key != undefined) return [data.matches_key]
    if (data.matches_keys != undefined) return data.matches_keys
    return []
}

const getOtherValues = (data) => {
    if (data.matches_value != undefined) return [data.matches_value]
    if (data.matches_values != undefined) return data.matches_values
    return []
}
...
const queryKey = async (keys) => {
    ...
    for(const key of keys) {
        ...
        responses.push( parseData(taginfoData.data, taginfoData.data_until) )
    }
...
const combineResponses = (arr) => {
    const date = (arr.length > 0) ? arr[0].date : ""
